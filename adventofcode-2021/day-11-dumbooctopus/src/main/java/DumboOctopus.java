...
public class DumboOctopus {
	...
	private static void updateValuesUponFlashes() {
		...
		for ( int y = 0; y < octopusMatrix.length; y++ ) {
			Octopus[] currentRow = octopusMatrix[y];
			for ( int x = 0; x < currentRow.length; x++ ) {
				Octopus currentOctopus = currentRow[x];
				if ( currentOctopus.getValue() == 0) {
					processFlashUpdating(currentOctopus);
					currentOctopus.setFlashed( true );
				}
			}
		}
		while( Arrays.stream( octopusMatrix ).anyMatch( x -> Arrays.stream( x ).anyMatch( y -> y.isToBeFlashed()
				&& !y.isFlashed() ) )){
			for ( int y = 0; y < octopusMatrix.length; y++ ) {
				Octopus[] currentRow = octopusMatrix[y];
				for ( int x = 0; x < currentRow.length; x++ ) {
	...
	private static void processFlashUpdating(Octopus octopus){
		numberOfFlashes++;
		for ( Directions d : Directions.values() ) {
			//check that an octopus (cell) is within the array range
			if ( ( octopus.getY() + d.getY() >= 0 && octopus.getY() + d.getY() < matrixHeightIndex ) && ( octopus.getX() + d
					.getX() >= 0 &&  octopus.getX() + d.getX() < matrixWidthIndex ) ) {
				//calculate the flashing value
//				if ( octopus.getValue() > 0 ) {
//					octopus.setValue(
//							octopus.getValue() + 1 );
					//FLASH!
					if ( octopus.getValue() > 9 ) {
						octopus.setValue( 0 );
						octopus.setToBeFlashed( true );
//					}
				}
				octopus.setToBeFlashed( false );
				octopusMatrix[ octopus.getY() + d.getY()][ octopus.getX() + d.getX()] = octopus;
			}
		}
	}

	private static void resetOctopuses() {
		for ( Octopus[] octopuses : octopusMatrix ) {
			for ( Octopus o : octopuses ) {
				o.setFlashed( false );
				o.setToBeFlashed( false );
			}
		}
	}
	...
	private static void generateOctopusMatrix() {
		...
		for ( int i = 0; i < matrixHeightIndex; i++ ) {
			...
			int[] stringArrConvertedToInArr = FileReaderUtil.convertStringOfDigitsToIntArr(
					currentLine );
	...
	private static List<String> readInitialState() {
		return FileReaderUtil.readFileAsLinesOfStrings( DumboOctopus.class.getClassLoader(),
				"test-input.txt" );
	}
